...
!!NOAe means that we assume zero aerosols and that all the loops with tr_aero as an input can be dropped
!!NOAe       if( tr_aero ) then
!!NOAe          ! check 4 layers for each aerosol, a snow SSL, snow below SSL,
!!NOAe          ! sea ice SSL, and sea ice below SSL, in that order.
!!NOAe          if (size(aero) < 4*n_aero) then
!!NOAe    call icepack_warnings_add(subname//' ERROR: size(aero) too small')
!!NOAe             call icepack_warnings_setabort(.true.,__FILE__,__LINE__)
!!NOAe             return
!!NOAe          endif
!!NOAe          do na = 1, 4*n_aero, 4
!!NOAe             vsno = hs * aice
!!NOAe             netsw = swvdr + swidr + swvdf + swidf
!!NOAe             if (netsw > puny) then ! sun above horizon
!!NOAe                aero_mp(na  ) = aero(na  )*vsno
!!NOAe                aero_mp(na+1) = aero(na+1)*vsno
!!NOAe                aero_mp(na+2) = aero(na+2)*vice
!!NOAe                aero_mp(na+3) = aero(na+3)*vice
!!NOAe             endif                  ! aice > 0 and netsw > 0
!!NOAe          enddo      ! na
!!NOAe       endif      ! if aerosols
...
!mgf++
!!NOAe      if (modal_aero) then
!!NOAe           do k=0,klev   
!!NOAe              if (k < nslyr+1) then ! define indices for snow layer
!!NOAe                 ! use top rsnw, rhosnw for snow ssl and rest of top layer
!!NOAe                 ksnow = k - min(k-1,0)
!!NOAe                 tmp_gs = frsnw(ksnow)
!!NOAe                
!!NOAe                 ! get grain size index:
!!NOAe                 ! works for 25 < snw_rds < 1625 um:
!!NOAe                 if (tmp_gs < 125) then
!!NOAe                   tmp1 = tmp_gs/50
!!NOAe                   k_bcini(k) = nint(tmp1)
!!NOAe                 elseif (tmp_gs < 175) then
!!NOAe                   k_bcini(k) = 2
!!NOAe                 else
!!NOAe                   tmp1 = (tmp_gs/250)+2
!!NOAe                   k_bcini(k) = nint(tmp1)
!!NOAe                 endif
!!NOAe              else                  ! use the largest snow grain size for ice
!!NOAe                 k_bcini(k) = 8
!!NOAe              endif
!!NOAe              ! Set index corresponding to BC effective radius.  Here,
!!NOAe              ! asssume constant BC effective radius of 100nm
!!NOAe              ! (corresponding to index 2)
!!NOAe              k_bcins(k) = 2
!!NOAe              k_bcexs(k) = 2
!!NOAe
!!NOAe              ! check bounds:
!!NOAe              if (k_bcini(k) < 1)  k_bcini(k) = 1
!!NOAe              if (k_bcini(k) > 8)  k_bcini(k) = 8
!!NOAe              if (k_bcins(k) < 1)  k_bcins(k) = 1
!!NOAe              if (k_bcins(k) > 10) k_bcins(k) = 10
!!NOAe              if (k_bcexs(k) < 1)  k_bcexs(k) = 1
!!NOAe              if (k_bcexs(k) > 10) k_bcexs(k) = 10
!!NOAe
!!NOAe              ! print ice radius index:
!!NOAe              ! write(warnstr,*) subname, "MGFICE2:k, ice index= ",k,  k_bcini(k)
!!NOAe              ! call icepack_warnings_add(warnstr)
!!NOAe            enddo   ! k

!!NOAe        if (tr_zaero .and. dEdd_algae) then ! compute kzaero for chlorophyll  
!!NOAe        do n = 1,n_zaero        
!!NOAe           if (n == 1) then ! interstitial BC
!!NOAe            do k = 0, klev
!!NOAe               do ns = 1,nspint   ! not weighted by aice
!!NOAe                  tzaer(ns,k) = tzaer(ns,k)+kaer_bc_tab(ns,k_bcexs(k))* &
!!NOAe                                zbio(nlt_zaero_sw(n)+k)*dzk(k)
!!NOAe                  wzaer(ns,k) = wzaer(ns,k)+kaer_bc_tab(ns,k_bcexs(k))* &
!!NOAe                                waer_bc_tab(ns,k_bcexs(k))* &
!!NOAe                                zbio(nlt_zaero_sw(n)+k)*dzk(k)
!!NOAe                  gzaer(ns,k) = gzaer(ns,k)+kaer_bc_tab(ns,k_bcexs(k))* &
!!NOAe                                waer_bc_tab(ns,k_bcexs(k))* &
!!NOAe                                gaer_bc_tab(ns,k_bcexs(k))*zbio(nlt_zaero_sw(n)+k)*dzk(k)
!!NOAe               enddo  ! nspint
!!NOAe            enddo
!!NOAe           elseif (n==2) then ! within-ice BC
!!NOAe            do k = 0, klev
!!NOAe               do ns = 1,nspint  
!!NOAe                  tzaer(ns,k) = tzaer(ns,k)+kaer_bc_tab(ns,k_bcins(k))  * &
!!NOAe                                bcenh(ns,k_bcins(k),k_bcini(k))* &
!!NOAe                                zbio(nlt_zaero_sw(n)+k)*dzk(k)
!!NOAe                  wzaer(ns,k) = wzaer(ns,k)+kaer_bc_tab(ns,k_bcins(k))* &
!!NOAe                                waer_bc_tab(ns,k_bcins(k))* &
!!NOAe                                zbio(nlt_zaero_sw(n)+k)*dzk(k)
!!NOAe                  gzaer(ns,k) = gzaer(ns,k)+kaer_bc_tab(ns,k_bcins(k))* &
!!NOAe                                waer_bc_tab(ns,k_bcins(k))* &
!!NOAe                                gaer_bc_tab(ns,k_bcins(k))*zbio(nlt_zaero_sw(n)+k)*dzk(k)
!!NOAe               enddo  ! nspint
!!NOAe            enddo
!!NOAe           else                ! dust
!!NOAe            do k = 0, klev
!!NOAe               do ns = 1,nspint   ! not weighted by aice
!!NOAe                  tzaer(ns,k) = tzaer(ns,k)+kaer_tab(ns,n)* &
!!NOAe                                   zbio(nlt_zaero_sw(n)+k)*dzk(k)
!!NOAe                  wzaer(ns,k) = wzaer(ns,k)+kaer_tab(ns,n)*waer_tab(ns,n)* &
!!NOAe                                   zbio(nlt_zaero_sw(n)+k)*dzk(k)
!!NOAe                  gzaer(ns,k) = gzaer(ns,k)+kaer_tab(ns,n)*waer_tab(ns,n)* &
!!NOAe                                   gaer_tab(ns,n)*zbio(nlt_zaero_sw(n)+k)*dzk(k)
!!NOAe               enddo  ! nspint
!!NOAe            enddo
!!NOAe           endif      !(n=1)
!!NOAe        enddo         ! n_zaero
!!NOAe        endif         ! tr_zaero and dEdd_algae

!!NOAe     else  ! Bulk aerosol treatment
!!NOAe        if (tr_zaero .and. dEdd_algae) then ! compute kzaero for chlorophyll
!!NOAe        do n = 1,n_zaero          ! multiply by aice?
!!NOAe            do k = 0, klev
!!NOAe               do ns = 1,nspint   ! not weighted by aice
!!NOAe                  tzaer(ns,k) = tzaer(ns,k)+kaer_tab(ns,n)* &
!!NOAe                                   zbio(nlt_zaero_sw(n)+k)*dzk(k)
...
!!NOAe            ! aerosol in snow
!!NOAe             if (tr_zaero .and. dEdd_algae) then 
!!NOAe               do k = 0,nslyr
!!NOAe                  gzaer(ns,k) = gzaer(ns,k)/(wzaer(ns,k)+puny)
!!NOAe                  wzaer(ns,k) = wzaer(ns,k)/(tzaer(ns,k)+puny)
!!NOAe                  g(k)   = (g(k)*w0(k)*tau(k) + gzaer(ns,k)*wzaer(ns,k)*tzaer(ns,k)) / &
!!NOAe                                  (w0(k)*tau(k) + wzaer(ns,k)*tzaer(ns,k))
!!NOAe                  w0(k)  = (w0(k)*tau(k) + wzaer(ns,k)*tzaer(ns,k)) / &
!!NOAe                                   (tau(k) + tzaer(ns,k))
!!NOAe                  tau(k) = tau(k) + tzaer(ns,k)
!!NOAe               enddo
!!NOAe             elseif (tr_aero) then
!!NOAe                  k = 0  ! snow SSL
!!NOAe               taer = c0
!!NOAe               waer = c0
!!NOAe               gaer = c0
...
!!NOAe               if (modal_aero) then
!!NOAe                  if (na == 1) then  
!!NOAe                  !interstitial BC
!!NOAe                     taer = taer + &
!!NOAe                          aero_mp(na)*kaer_bc_tab(ns,k_bcexs(k))
!!NOAe                     waer = waer + &
!!NOAe                          aero_mp(na)*kaer_bc_tab(ns,k_bcexs(k))* &
!!NOAe                          waer_bc_tab(ns,k_bcexs(k))
!!NOAe                     gaer = gaer + &
!!NOAe                          aero_mp(na)*kaer_bc_tab(ns,k_bcexs(k))* &
!!NOAe                           waer_bc_tab(ns,k_bcexs(k))*gaer_bc_tab(ns,k_bcexs(k))
!!NOAe                  elseif (na == 5)then  
!!NOAe                  !within-ice BC
!!NOAe                      taer = taer + &
!!NOAe                           aero_mp(na)*kaer_bc_tab(ns,k_bcins(k))* &
!!NOAe                           bcenh(ns,k_bcins(k),k_bcini(k))
!!NOAe                      waer = waer + &
!!NOAe                           aero_mp(na)*kaer_bc_tab(ns,k_bcins(k))* &
!!NOAe                           waer_bc_tab(ns,k_bcins(k))
!!NOAe                      gaer = gaer + &
!!NOAe                           aero_mp(na)*kaer_bc_tab(ns,k_bcins(k))* &
!!NOAe                           waer_bc_tab(ns,k_bcins(k))*gaer_bc_tab(ns,k_bcins(k))
!!NOAe                   else
!!NOAe                      ! other species (dust)
!!NOAe                      taer = taer + &
!!NOAe                           aero_mp(na)*kaer_tab(ns,(1+(na-1)/4))
!!NOAe                      waer = waer + &
!!NOAe                           aero_mp(na)*kaer_tab(ns,(1+(na-1)/4))* &
!!NOAe                           waer_tab(ns,(1+(na-1)/4))
!!NOAe                      gaer = gaer + &
!!NOAe                           aero_mp(na)*kaer_tab(ns,(1+(na-1)/4))* &
!!NOAe                           waer_tab(ns,(1+(na-1)/4))*gaer_tab(ns,(1+(na-1)/4))
!!NOAe                   endif
!!NOAe               else
!!NOAe                  taer = taer + &
!!NOAe                       aero_mp(na)*kaer_tab(ns,(1+(na-1)/4))
...
!!NOAe!mgf--
!!NOAe               enddo       ! na
!!NOAe               gaer = gaer/(waer+puny)
!!NOAe               waer = waer/(taer+puny)
!!NOAe
!!NOAe               do k=1,nslyr
!!NOAe                  taer = c0
!!NOAe                  waer = c0
...
!!NOAe                      ! interstitial BC
!!NOAe                      taer = taer + &
!!NOAe                           (aero_mp(na+1)/rnslyr)*kaer_bc_tab(ns,k_bcexs(k))
!!NOAe                      waer = waer + &
!!NOAe                           (aero_mp(na+1)/rnslyr)*kaer_bc_tab(ns,k_bcexs(k))* &
!!NOAe                           waer_bc_tab(ns,k_bcexs(k))
!!NOAe                      gaer = gaer + &
...
!!NOAe                      ! within-ice BC
!!NOAe                      taer = taer + &
!!NOAe                           (aero_mp(na+1)/rnslyr)*kaer_bc_tab(ns,k_bcins(k))*&
!!NOAe                           bcenh(ns,k_bcins(k),k_bcini(k))
!!NOAe                      waer = waer + &
!!NOAe                           (aero_mp(na+1)/rnslyr)*kaer_bc_tab(ns,k_bcins(k))* &
!!NOAe                           waer_bc_tab(ns,k_bcins(k))
...
!!NOAe                      ! other species (dust)
!!NOAe                      taer = taer + &
!!NOAe                           (aero_mp(na+1)/rnslyr)*kaer_tab(ns,(1+(na-1)/4))
!!NOAe                      waer = waer + &
!!NOAe                           (aero_mp(na+1)/rnslyr)*kaer_tab(ns,(1+(na-1)/4))* &
!!NOAe                           waer_tab(ns,(1+(na-1)/4))
!!NOAe                      gaer = gaer + &
...
!!NOAe                  gaer = gaer/(waer+puny)
!!NOAe                  waer = waer/(taer+puny)
!!NOAe                  g(k)   = (g(k)*w0(k)*tau(k) + gaer*waer*taer) / &
!!NOAe                           (w0(k)*tau(k) + waer*taer)
!!NOAe                  w0(k)  = (w0(k)*tau(k) + waer*taer) / &
!!NOAe                           (tau(k) + taer)
!!NOAe                  tau(k) = tau(k) + taer
!!NOAe               enddo       ! k
!!NOAe            endif     ! tr_aero
...
!!NOAe                  ! interstitial BC
!!NOAe                     taer = taer + &
!!NOAe                          aero_mp(na+2)*kaer_bc_tab(ns,k_bcexs(k))
!!NOAe                     waer = waer + &
!!NOAe                          aero_mp(na+2)*kaer_bc_tab(ns,k_bcexs(k))* &
!!NOAe                          waer_bc_tab(ns,k_bcexs(k))
!!NOAe                     gaer = gaer + &
...
!!NOAe                  ! within-ice BC
!!NOAe                     taer = taer + &
!!NOAe                          aero_mp(na+2)*kaer_bc_tab(ns,k_bcins(k))* &
!!NOAe                          bcenh(ns,k_bcins(k),k_bcini(k))
!!NOAe                     waer = waer + &
!!NOAe                          aero_mp(na+2)*kaer_bc_tab(ns,k_bcins(k))* &
!!NOAe                          waer_bc_tab(ns,k_bcins(k))
...
!!NOAe                  ! other species (dust)
!!NOAe                     taer = taer + &
!!NOAe                          aero_mp(na+2)*kaer_tab(ns,(1+(na-1)/4))
!!NOAe                     waer = waer + &
!!NOAe                          aero_mp(na+2)*kaer_tab(ns,(1+(na-1)/4))* &
!!NOAe                          waer_tab(ns,(1+(na-1)/4))
!!NOAe                     gaer = gaer + &
...
!!NOAe                        ! interstitial BC
!!NOAe                        taer = taer + &
!!NOAe                             (aero_mp(na+3)/rnilyr)*kaer_bc_tab(ns,k_bcexs(k))
!!NOAe                        waer = waer + &
!!NOAe                             (aero_mp(na+3)/rnilyr)*kaer_bc_tab(ns,k_bcexs(k))* &
!!NOAe                             waer_bc_tab(ns,k_bcexs(k))
!!NOAe                        gaer = gaer + &
...
!!NOAe                        ! within-ice BC
!!NOAe                        taer = taer + &
!!NOAe                             (aero_mp(na+3)/rnilyr)*kaer_bc_tab(ns,k_bcins(k))* &
!!NOAe                             bcenh(ns,k_bcins(k),k_bcini(k))
!!NOAe                        waer = waer + &
!!NOAe                             (aero_mp(na+3)/rnilyr)*kaer_bc_tab(ns,k_bcins(k))* &
!!NOAe                             waer_bc_tab(ns,k_bcins(k))
...
!!NOAe                        ! other species (dust)
!!NOAe                        taer = taer + &
!!NOAe                             (aero_mp(na+3)/rnilyr)*kaer_tab(ns,(1+(na-1)/4))
!!NOAe                        waer = waer + &
!!NOAe                             (aero_mp(na+3)/rnilyr)*kaer_tab(ns,(1+(na-1)/4))* &
!!NOAe                             waer_tab(ns,(1+(na-1)/4))
!!NOAe                        gaer = gaer + &
